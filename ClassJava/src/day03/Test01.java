...
//		}
		 ...
		 * 
		 * 	주의사항]
		 * 		반드시 만들 때 관리할 타입과 길이를 정해야 된다.
		 * 		한번 만들어진 배열은 길이를 수정 못한다.
		 * 
		 * 	형식]
		 * 		데이터 타입[] 변수이름 = {데이터1, 데이터2, ...};
		 * 		데이터 타입[] 변수이름 = new 데이터타입[길이];
		 * 		데이터 타입[] 변수이름 = new 데이터타입[]{데이터1, 데이터2, ...};
		 * 
		 * 	참고]
		 * 		배열의 내용을 확인하는 함수
		...
		System.out.println("향상된 for 명령으로 출력해보자.");
		for(int arrNum : num) {
		...
		// 따라서 배열의 주소를 기억하고 있는 변수를 입력해주면 된다.
